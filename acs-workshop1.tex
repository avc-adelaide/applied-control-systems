...
\item Spend a couple minutes experimenting with different values of $m$, $c$, $k$, and initial conditions (e.g. try doubling the mass, or halving the spring stiffness)
...
\begin{itemize}
\item Click on \Mbutton{Library Browser} and explore some of the available blocks
\item Double click in the empty space of your model and type `Sine Wave' to insert a sinusoid `asourcee'tw
...
\begin{itemize}
\item
Double click the Sine Wave block and change the amplitude to \CODE{AA} and the frequency to \CODE{ww}
\item
...
\begin{itemize}
\item
Add a new variable \CODE{tmax = 20;} and figure out how to use it to set the `Stop Time' of the Simulink model
\item
You can now set up a model in Simulink and produce results in a nicely general way
\end{itemize}
...
\item E.g., \Mbutton{Solver details} > \Mbutton{Relative tolerance} = \texttt{1e-6}\\
