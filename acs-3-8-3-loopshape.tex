...
\begin{frame}{Desirable loop transfer function shape}
\begin{figure}
	\centering
...
\begin{itemize}
	\item The robustness of the system is determined by the shape of the loop transfer function around the crossover frequency $\ww\gaincrossover$
	\item It is desirable to transition from high loop gain at low frequencies to low loop gain quickly, but robustness requirements (see Bode's relations) impose restrictions on how fast the gain can decrease
	\item For a gain curve of constant slope around $\ww\gaincrossover$, the slope $n\gaincrossover$ is related to the phase margin $\phasemargin$ (in degrees) by
	\[n\gaincrossover \approx -2 + \frac{\phasemargin}{90}\]
