...
\begin{frame}{Characterising performance}
	\begin{itemize}
		...
		\item The maximum value of the sensitivity and complementary sensitivity functions, $M_s$ (occurring at $\omega_{ms}$) and $M_t$ (at $\omega_{mt}$), are also important
...
\begin{frame}{Tracking at low frequency}
	\begin{itemize}
		\item The transfer function $G_{yr}$ typically has unity zero frequency gain as we want the system to have step input with zero steady-state error
		...
		\[e(t)=r(t)-y(t)= \mathcal{L}^{-1} \{ G_{er}(s) R(s) \} = e_1 \frac{\mathrm{d}r}{\mathrm{d}t}+ e_2 \frac{\mathrm{d}^2r}{\mathrm{d}t^2}+ \cdots\]
...
\begin{itemize}
	...
	and a PI controller with error feedback having gains $\kkp=0.6$ and $\kki=0.5$
	\item We compare the performance of a controller with only error feedback, and  with feedforward control implemented to achieve $G_{yr}=\frac{1}{(0.5s + 1)^3}$
\end{itemize}
\begin{figure}
	\centering
...
\begin{itemize}
\item The sensitivity function $S(s)$ shows how feedback influences the response of the output to load disturbances
\item Disturbances with frequencies such that $|S(\ii\ww)|<1$ are attenuated and disturbances with frequencies with $|S(\ii\ww)|>1$ are amplified by feedback
\item The sensitivity crossover frequency $\ww_{\text{sc}}$ is the lowest frequency where $|S(\ii\ww)|=1$
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item The complex number $1+L(\ii\ww)$ is the inverse of $S(\ii\ww)$ and can be viewed as the vector from $-1$ to the point $L(\ii\ww)$ on the Nyquist curve
...
\begin{itemize}
	\item The maximum sensitivity $\Msens$ (which occurs at $\ww_{\text{ms}}$) is a measure of the largest amplification of disturbances
	\item It is equal to the inverse of the stability margin
	\[
	  \Msens =\frac{1}{s_{\text{m}}}
	\]
	\item The sensitivity crossover frequency $\ww_{\text{sc}}$ and the maximum sensitivity $\Msens$ give a characterisation of load disturbance attenuation
\end{itemize}
\end{frame}
...
\begin{frame}{Load disturbance transfer function}
	\begin{itemize}
		...
		\]
		\item As load disturbances typically have low frequencies, small $s$ results in $T\approx1$ and so $G_{yv}\approx \frac{1}{C}$ for small $s$
		\item Therefore, for a controller with integral action at small $s$, $G_{yv}\approx \frac{s}{\kki}$ and so disturbances are attenuated effectively
...
\begin{frame}{Measurement noise transfer function}
	\begin{itemize}
		...
		\item The TF from measurement noise $w$ to control signal $u$ is 
		\[
		    -G_{uw} = \frac{C}{1+PC}=CS=\frac{T}{P}
		\]
		\item Assuming that $S\approx 1$ for large $s$ (high frequencies), $-G_{uw}\approx C$ for large $s$
