...
\begin{itemize}
	...
	\[K=P(0)\]
	\item Using integral control, the controller is $K\tfrac{\kki}{s}$ with closed loop characteristic polynomial $s+K\kki$
	\item Specifying the performance of the controlled system by the desired time constant $T_{\text{cl}}$ of the closed loop system, we have
	\[\kki = \frac{1}{T_{\text{cl}} P(0)}\]
	\item A reasonable criterion for the process transfer function to be well approximated by a constant is that the average residence time $T_{\text{ar}}=\tfrac{-P'(0)}{P(0)}$ satisfies $T_{\text{ar}}<T_{\text{cl}}$
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\[P(s) \approx \frac{P(0)}{1+sT_{\text{ar}}}\]
	...
	\[\kki = \frac{1}{2P(0)T_{\text{ar}}}\]
	\item This controller has $\omega_0 = \tfrac{1}{T_{\text{ar}}\sqrt{2}}$, with rise time of $3.1 T_{\text{ar}}$, settling time of $7.9T_{\text{ar}}$, and overshoot of 4\%
\end{itemize}
\end{frame}
...
\item PI has two gains to set the CL poles for simple models
...
\[s^2+a_1s+a_2 = s^2+(a+b\kkp)s+b\kki\]
...
\begin{itemize}
	...
	\item A PID controller can be used to control the system to have arbitrary pole locations
