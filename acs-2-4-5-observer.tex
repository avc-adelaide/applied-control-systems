...
\begin{frame}{Observability matrix}
	\begin{itemize}
		...
		\end{bmatrix}\]
		\item A linear system is \alert{observable} if and only if the observability matrix $\Wob$ is full row rank (it has $n$ independent rows)
	\end{itemize}
\end{frame}
...
\begin{frame}{Observability canonical form}
	\begin{itemize}
		...
		\small{\[\tilde{W}_{\mathrm o} = \begin{bmatrix}
			1 & & & & \\
			a_1 & 1& & & \\
			a_2 & a_1 & 1 & & \\
			\vdots & \vdots & & \ddots & \\
...
\begin{frame}
\frametitle{The observer}
Observers are linear dynamical systems that take the inputs $u$ and outputs $y$ of a system and produces an estimate $\hat{x}$ of the system state $x$
...
\begin{itemize}
	...
	\[A \leftrightarrow A\Tr, \quad B \leftrightarrow C\Tr, \quad K \leftrightarrow L\Tr, \quad W_{\mathrm{r}} \leftrightarrow W_{\mathrm{o}}\Tr \]
...
\begin{frame}{Observer design}
	\begin{enumerate}
		...
		\[L= \Wob^{-1} \tilde{W}_{\mathrm o}\begin{bmatrix}
