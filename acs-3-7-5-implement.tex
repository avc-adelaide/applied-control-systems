...
\begin{frame}{Digital implementation}
	\begin{itemize}
		...
		\[I(t_{k+1}) = I(t_k)+\kki h e(t_k) + \tfrac{h}{T_{\text{aw}}} (\operatorname{sat}(u_a)-u_a)\]
		where $T_{\text{aw}}=h/k_{\text{aw}}$ represents the anti-windup term
	\end{itemize}
\end{frame}
...
\begin{frame}{Digital implementation}
	\begin{itemize}
		...
		\item Working close to the `silicon', it is common to formulate control problems using \alert{discrete domain} $z$-transform instead of continuous domain $s$-transform
	\end{itemize}
\end{frame}

\begin{frame}
\frametitle{`A Practical Guide to PID Controller Implementation'}
\begin{itemize}
\item \textcite{sundstrom2026}:
\end{itemize}
\begin{quote}
\parskip=4pt\relax
\scriptsize
How difficult can it be to implement a PID controller? The answer is twofold. 
\begin{itemize}
\item
Implementing the PID control law is simple and computationally inexpensive. However,
\item
this basic form will not work in practical applications. 
\end{itemize}

The primary reason for this
is the various physical limitations of the actuator.

Measurement noise, different implementations depending on the various structures (P, PI, PD or PID), bumpless
transfer, and varying sampling time also result in problems rendering the basic form
inoperable.

PID implementation is therefore more difficult than meets the eye. 
This paper introduces a reference implementation of the PID controller which considers
these practical issues.

It includes pseudo-code, discussion of the implementation
choices and simulation of carefully selected, important test cases.
\end{quote}
\end{frame}
...
\frametitle{`A Practical Guide to PID Controller Implementation'}
\centering
\includegraphics[width=0.8\linewidth]{sundstrom}
\end{frame}

\begin{frame}
\frametitle{With thanks to M. Bauer}

\centering
\includegraphics[width=0.8\linewidth]{iceberg}
