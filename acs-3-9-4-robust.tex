...
\[P(s) = \frac{b}{s+a}, \quad C(s)=\kkp+\frac{\kki}{s}\]
\item The closed loop characteristic polynomial is
\[s(s+a)+b(\kkp s+\kki)=s^2+(a+b\kkp)s+\kki b\]
\item If we want poles at $-p_1$ and $-p_2$ we get
\[\kkp=\frac{p_1+p_2-a}{b}, \quad \kki=\frac{p_1p_2}{b}\]
\end{itemize}
\end{frame}
...
\begin{frame}{Fast stable process poles}
	\begin{itemize}
		...
		\item Assume that pole $a$ is faster than the closed loop poles, the proportional gain $\kkp$ is then negative and the controller has a zero in the right half-plane
