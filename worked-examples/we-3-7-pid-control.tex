...
Using the final value theorem for unit step input $R(s) = 1/s$:
\begin{align}
y_{ss} &= \lim_{s \to 0} s \cdot T_P(s) \cdot \frac{1}{s} = \lim_{s \to 0} \frac{4}{s^2 + 2s + 4} = \frac{4}{4} = 1
\end{align}

Steady-state error: $e_{ss} = 1 - 1 = 0$

Wait, this is wrong because the plant has an integrator. Let me recalculate.

Actually, for a Type-1 system (one integrator in the loop), the steady-state error to a step is:
\begin{align}
