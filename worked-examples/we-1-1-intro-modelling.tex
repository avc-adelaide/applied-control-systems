...
\subsection{Example 2: State-Space from Transfer Function}

\paragraph{Problem:} Convert the transfer function:
\begin{align}
G(s) = \frac{Y(s)}{U(s)} = \frac{3s + 6}{s^2 + 4s + 3}
\end{align}

to state-space form using the controllable canonical form.

\paragraph{Solution:}

\textbf{Step 1: Identify the transfer function structure}

The transfer function can be written as:
\begin{align}
G(s) = \frac{b_1 s + b_0}{s^2 + a_1 s + a_0} = \frac{3s + 6}{s^2 + 4s + 3}
\end{align}

where $b_1 = 3$, $b_0 = 6$, $a_1 = 4$, $a_0 = 3$.

\textbf{Step 2: Controllable canonical form}

For a second-order system, the controllable canonical form is:
\begin{align}
A = \begin{bmatrix} 0 & 1 \\ -a_0 & -a_1 \end{bmatrix}, \quad B = \begin{bmatrix} 0 \\ 1 \end{bmatrix}, \quad C = \begin{bmatrix} b_0 - b_1 a_0 & b_1 \end{bmatrix}, \quad D = 0
\end{align}

Wait, let me reconsider. For proper controllable canonical form where the numerator has degree less than the denominator, we need to check if there's direct feedthrough.

Since the numerator degree (1) is less than the denominator degree (2), we have $D = 0$ and proceed with:

\begin{align}
A &= \begin{bmatrix} 0 & 1 \\ -a_0 & -a_1 \end{bmatrix} = \begin{bmatrix} 0 & 1 \\ -3 & -4 \end{bmatrix} \\
B &= \begin{bmatrix} 0 \\ 1 \end{bmatrix} \\
C &= \begin{bmatrix} b_0 & b_1 \end{bmatrix} = \begin{bmatrix} 6 & 3 \end{bmatrix} \\
D &= 0
\end{align}

\textbf{Step 3: Verification}

Let's verify by computing $G(s) = C(sI - A)^{-1}B + D$:
\begin{align}
sI - A = \begin{bmatrix} s & -1 \\ 3 & s+4 \end{bmatrix}
\end{align}

\begin{align}
(sI - A)^{-1} = \frac{1}{s(s+4) + 3}\begin{bmatrix} s+4 & 1 \\ -3 & s \end{bmatrix} = \frac{1}{s^2+4s+3}\begin{bmatrix} s+4 & 1 \\ -3 & s \end{bmatrix}
\end{align}

\begin{align}
(sI - A)^{-1}B = \frac{1}{s^2+4s+3}\begin{bmatrix} s+4 & 1 \\ -3 & s \end{bmatrix}\begin{bmatrix} 0 \\ 1 \end{bmatrix} = \frac{1}{s^2+4s+3}\begin{bmatrix} 1 \\ s \end{bmatrix}
\end{align}

\begin{align}
G(s) = C(sI-A)^{-1}B = \begin{bmatrix} 6 & 3 \end{bmatrix}\frac{1}{s^2+4s+3}\begin{bmatrix} 1 \\ s \end{bmatrix} = \frac{6 + 3s}{s^2+4s+3} = \frac{3s+6}{s^2+4s+3}
\end{align}
...
\textbf{Key Insights:}
\begin{itemize}
\item Transfer functions can be converted to state-space form in multiple ways (controllable canonical, observable canonical, diagonal form)
\item The controllable canonical form is useful for controller design
\item State-space and transfer function representations are equivalent for linear time-invariant systems
\item The choice of state variables is not unique — different realizations have the same input-output behavior
\end{itemize}
