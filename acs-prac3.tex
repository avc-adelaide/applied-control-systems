...
\begin{frame}{Understanding proportional feedback}
	You can use a second signal scope to show the motor control signal and see the `proportional error'. Further, you can change the scope units to degrees!
    \begin{figure}
		\centering
...
\begin{enumerate}
	...
	\item As we are working with discretised values of angle from the angular encoder, we must first filter our signal using a low-pass filter as we saw in Week 1, choose an appropriate cutoff frequency $f_c$ = $\omega_c/2\pi$
...
\begin{frame}{Understanding derivative feedback}
	Add the `derivative error' to your scope.
    \begin{figure}
		\centering
...
\begin{frame}{Understanding integral feedback}
	Add the `integral error' to your scope.
    \begin{figure}
		\centering
...
\begin{frame}{Alternate block diagram}
    \begin{figure}
		\centering
		\includegraphics[width=10cm]{prac3_PID_BD.PNG}
